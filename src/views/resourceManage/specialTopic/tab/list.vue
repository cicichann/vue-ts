<template>
  <div class="info-list">
    ...
    <div class="sub-nav">
      <div class="fl">
        <router-link :to="{path: '/manage/topic/home',
                           query: $pick($route.query, ['standardid', 'categoryid', 'categoryname', 'hasChildren', 'parentid', 'searchText', 'searchType', 'topicid'])}"
                     tag="button" class="edit-btn other-btn back-btn">返回</router-link>
        <button v-if="checkRight(status.selectedIds, 'datamgt.dissertation.withdraw')"
                :disabled="status.selectedIds.length === 0" class="edit-btn delete-btn" @click.prevent="deleteClick(status.selectedIds)">撤稿</button>
        <dropdown-btn v-if="checkRight(status.selectedIds, 'datamgt.dissertation.changeCategory,datamgt.dissertation.addCategory')" :disabled="status.selectedIds.length === 0" label="调整分类">
          <ul>
            <li v-if="checkRight(status.selectedIds, 'datamgt.dissertation.changeCategory')" @click="changeCategory('replace')">替换分类</li>
            <li v-if="checkRight(status.selectedIds, 'datamgt.dissertation.addCategory')" @click="changeCategory('add')">追加分类</li>
          </ul>
        </dropdown-btn>
        <dropdown-btn v-if="checkRight(status.selectedIds, 'datamgt.dissertation.changeLabel,datamgt.dissertation.addLabel')" :disabled="status.selectedIds.length === 0" label="调整标签">
          <ul>
            <li v-if="checkRight(status.selectedIds, 'datamgt.dissertation.changeLabel')" @click="changeLabel('replace')">替换标签</li>
            <li v-if="checkRight(status.selectedIds, 'datamgt.dissertation.addLabel')" @click="changeLabel('add')">追加标签</li>
          </ul>
        </dropdown-btn>
      </div>
    ...
    <div class="sub-filter">
      <ul class="manage-tag fl">
        <router-link :to="{path: '/manage/topic/list', query: $route.query}" title="文档" tag="li" class="tag-list active"></router-link>
        <router-link :to="{path: '/manage/topic/pic', query: $route.query}" title="图片" tag="li" class="tag-pic"></router-link>
        <router-link :to="{path: '/manage/topic/video', query: $route.query}" title="视频" tag="li" class="tag-video"></router-link>
    ...
    <div class="table-content flex-table">
      ...
      <com-table
        ...
        <template slot-scope="item">
          <button v-if="checkRight(String(item.row.id), 'datamgt.dissertation.withdraw')" class="list-icon undo-icon" title="撤稿" @click.stop="deleteClick(item.row.id, item.row.isOrigin)"/>
          <el-popover v-if="checkRight(String(item.row.id), 'datamgt.dissertation.preview')" v-model="item.row.isShowAI"
                      popper-class="intelligent-popover" placement="bottom" width="90">
            <ul>
...
export default {
  ...
  methods: {
     ...
     */
    initSelected () {
      this.data.labels = this.topicMsg.labels.map(label => {
        label.label = label.labelName
        return label
      })
      this.status.selectedLabels = this.topicMsg.labels.map(label => label.labelName)
      this.status.selectedCategories = this.topicMsg.categories.map(category => ({id: category.id, name: category.categoryName}))

      // 专题没有设置标签时，默认展示全部标签
      if (this.data.labels.length === 0) this.queryLabels()
    },
            ...
            .map((item) => {
              ...
              if (item.is_title === '是' ) {
                ...
                column.render = (row, index, prop) => {
                  ...
                  let content = this.checkRight(String(row.id), 'datamgt.dissertation.preview') ? 
    ...
    formatContent (item, row, result) {
      ...
      // 标题字段点击跳转预览页面
      // if (item.is_title === '是') {
      //   let hasRight = this.checkRight(String(row.id), 'datamgt.dissertation.preview')
    ...
    queryList(page) {
      ...
      let url = `/standards/${this.$route.query.standardid}/dissertations/${this.$route.query.topicid}/resources/${this.$route.query.resourceid}/data`
    ...
    querySourceRight (dataIds, categoryIds) {
      ...
      return new Promise(resolve => {
        if (this.$route.query.standardid && this.$route.query.resourceid) {
          this.$api.get(`/standards/${this.$route.query.standardid}/dissertations/resources/${this.$route.query.resourceid}/data/rights/keys`, params).then(data => {
    ...
    changeLabel (type) {
      let option = {
        dataIds: this.status.selectedIds,
        standardId: this.$route.query.standardid,
        resourceId: this.$route.query.resourceid,
        containsAll: this.$refs.table.selectType === 'all' ? 1 : 0,
        type: type
      ...
      this.$dialog(labelChange, option).then(() => {
        this.queryList()
        if (this.topicMsg.labels.length === 0) this.queryLabels()
      })
    },
