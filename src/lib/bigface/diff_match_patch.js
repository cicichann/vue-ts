...
let diffMatch = function() {
      ...
      for(n=-v+p; n<=v-w; n+=2) {
        ...
        if(u>d) { w+=2; }
          ...
          if(m>e) { p+=2; }
            ...
            if(!q&&(l=g+k-n,0<=l&&(l<h&&-1!=j[l])&&(m=j[l],s=g+m-l,u=d-u,m>=u)))return this.diff_bisectSplit_(a,b,m,s,c)
            }
    }
    ...
    for(f=1; f<a.length;) {
      if(-1==a[f-1][0]&&1==a[f][0]) {
        ...
        if(d>=e) {
          ...
          if(e>=b.length/2||e>=c.length/2) { a.splice(f,0,[0,b.substring(0,e)]),a[f-1][0]=1,a[f-1][1]=c.substring(0,c.length-e),a[f+1][0]=-1,a[f+1][1]=b.substring(e),f++; }
  ...
  diff_match_patch.prototype.diff_cleanupEfficiency=function(a) {
    for(var b=!1,c=[],d=0,e=null,f=0,g=!1,h=!1,j=!1,i=!1; f<a.length;) {
      if(0==a[f][0]) { a[f][1].length<this.Diff_EditCost&&(j||i)?(c[d++]=f,g=j,h=i,e=a[f][1]):(d=0,e=null),j=i=!1; }
        ...
        if(-1==a[f][0]?i=!0:j=!0,e&&(g&&h&&j&&i||e.length<this.Diff_EditCost/2&&3==g+h+j+i)) { a.splice(c[d-1],0,[-1,e]),a[c[d-1]+1][0]=1,d--,e=null,g&&h?(j=i=!0,d=0):(d--,f=0<d?c[d-1]:-1,j=i=!1),b=!0; }
  ...
  diff_match_patch.prototype.patch_make=function(a,b,c) {
    ...
    if("string"==typeof a&&"string"==typeof b&&"undefined"==typeof c) { d=a,b=this.diff_main(d,b,!0),2<b.length&&(this.diff_cleanupSemantic(b),this.diff_cleanupEfficiency(b)); }
      ...
      if(a&&"object"==typeof a&&"undefined"==typeof b&&"undefined"==typeof c) { b=a,d=this.diff_text1(b); }
      else
        if("string"==typeof a&&b&&"object"==typeof b&&"undefined"==typeof c) { d=a; }
        else
          if("string"==typeof a&&"string"==typeof b&&c&&"object"==typeof c) { d=a,b=c; }
  ...
  diff_match_patch.prototype.patch_apply=function(a,b) {
    ...
    for(var d=0,e=[],f=0; f<a.length; f++) {
      ...
      if(-1==j) { e[f]=!1,d-=a[f].length2-a[f].length1; }
        ...
        if(e[f]=!0,d=j-g,g=-1==i?b.substring(j,j+h.length):b.substring(j,i+this.Match_MaxBits),h==g) { b=b.substring(0,j)+this.diff_text2(a[f].diffs)+b.substring(j+h.length); }
        else
          if(g=this.diff_main(h,g,!1),h.length>this.Match_MaxBits&&this.diff_levenshtein(g)/h.length>this.Patch_DeleteThreshold) { e[f]=!1; }
          else {
      ...
      for(c++; c<a.length;) {
        ...
        if("-"==e) { f.diffs.push([-1,g]); }
          ...
          if("+"==e) { f.diffs.push([1,g]); }
            ...
            if(" "==e) { f.diffs.push([0,g]); }
            else
              if("@"==e) { break; }
              else
                if(""!==e) { throw Error('Invalid patch mode "'+e+'" in: '+g); }
