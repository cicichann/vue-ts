<template>
  <div class="type-select">
    <!-- 字段类型 -->
    <el-select v-model="type" :disabled="!!columnId" placeholder="请选择字段类型" @change="handleTypeChange">
    ...
    <el-form-item v-if="type === 'TEXT'" class="in-input-row in-sub-row" label="默认值" prop="defaultValue">
      <el-input :disabled="isSystemField" v-model.trim="formData.defaultValue" placeholder="请输入默认值"/>
    </el-form-item>
    ...
    <!-- 复合字段 -->
    <template v-if="type === 'ARRAY' || type === 'OBJECT'">
      <el-form-item class="in-input-row" style="margin-top: 10px;">
        <ul class="in-item-setting in-select-items">
          <li v-for="(item, index) in formData.compositeEnum" :key="item.cn_name" class="draggable">
            <i class="move-icon"/><el-input v-model="item.cn_name" readonly placeholder="子字段中文名" />
            <i class="edit-icon" @click="editItem(item, index)" />
            <i v-if="formData.compositeEnum.length > 1" class="delete-icon" @click="removeItem(index, item)" />
          </li>
          <li class="in-add-item dis-draggable" style="text-align: center;" @click="addItem"><i class="add-icon"/><span>新增子字段</span></li>
        </ul>
      </el-form-item>
    </template>
    ...
    <el-form-item v-if="type === 'DATE'" class="in-input-row in-sub-row" label="默认值" prop="dateValue">
      <el-radio-group v-model="formData.time" @change="handleDateChange">
        <el-radio :disabled="isSystemField" label="empty">无</el-radio>
        <el-radio :disabled="isSystemField" label="systemTime">系统时间</el-radio>
        <el-radio :disabled="isSystemField" label="customTime">自定义
          <el-date-picker v-model="formData.dateCustomeValue" :default-value="formData.dateCustomeValue" :editable="false"
                          :class="{'is-visible': formData.time === 'customTime'}" format="yyyy-MM-dd" type="date" placeholder="选择日期" @focus="handleDateFocus" @change="handleDateChange"/>
        </el-radio>
      </el-radio-group>
    </el-form-item>
  </div>
...
import editSubFieldDialog from './editSubField.vue'
...
export default {
  ...
  computed: {
    isSystemField () {
      ...
      let sysFieldArr = ['category', 'label', 'data_sources', 'source_code', 'save_time']
      return sysFieldArr.indexOf(enName) >= 0 || enName === ''
    },
  ...
  watch: {
    type (curVal) {
      ...
      if (this.isEnumerable() || this.isComposite()) {
        setTimeout(() => {
          this.rowDrop()
        }, 200)
      }
  ...
  methods: {
    ...
    initData () {
      ...
      let fieldsValue = this.fieldData.fields
      if (type === 'CHECK') {
        this.formData.checkDefault = (defaultValue && defaultValue.split(',').map((value) => { return parseInt(value) })) || []
      } else if (type === 'RADIO' || type === 'SELECT') {
        this.formData[type.toLowerCase() + 'Default'] = parseInt(defaultValue) || ''
      } else if (type === 'DATE') {
        if (defaultValue === '') {
      ...
      if (fieldsValue) this.formData.compositeEnum = fieldsValue
    },
    ...
    removeItem (index, item) {
      if (this.isComposite()) {
        this.formData.compositeEnum.splice(index, 1)
      } else {
     ...
     */
    rowDrop () {
      const el = document.querySelector('.type-select ul.in-item-setting')
      // 实例化，定义初始配置
      Sortable.create(el, {
        group: 'index',
        sort: true,
        ...
        onEnd: (evt) => {
          ...
          if (evt.newIndex !== evt.oldIndex) {
            let enumValueList
            if (this.isComposite()) {
              enumValueList = this.formData.compositeEnum
            } else {
              enumValueList = this.formData[this.fieldData.field_type.toLowerCase() + 'Enum']
            }
     ...
     */
    handleDateChange () {
      let type = this.formData.time
      if (type === 'empty') {
        this.formData.dateDefault = ''
      } else {
        this.formData.dateDefault = type === 'systemTime' ? '0' : this.formData.dateCustomeValue.getTime() + ''
      }
    },
    ...
    getEnumValue () {
      let enumValueArr = []
      if (this.isComposite()) {
        enumValueArr = this.formData.compositeEnum
      } else {
        let enumValueList = this.formData[this.type.toLowerCase() + 'Enum']
     ...
     */
    getCompositeValue () {
      this.formData.compositeEnum.map((item, index) => {
        item.children_serial_number = index
      })
      return this.formData.compositeEnum
    },
    ...
    validation () {
      return new Promise((resolve, reject) => {
        if (this.isEnumerable() && this.getEnumValue().length < 2) {
          ...
          this.status.isErrorShow = true
          return reject()
        } else if (this.isComposite() && this.getEnumValue().length < 1) {
          this.error.errorMsg = this.error.compositeError
          this.status.isErrorShow = true
          return reject()
    ...
    editItem (item, index) {
      let props = {
        query: this.query,
        parentFieldData: this.fieldData,
        compositeList: this.formData.compositeEnum,
        editItem: item,
        index: index
      }
      this.$dialog(editSubFieldDialog, props).then((data) => {
        this.status.isErrorShow = false
        if (index || index === 0) {
          this.formData.compositeEnum.splice(index, 1, data.item)
        } else {
          this.formData.compositeEnum.push(data.item)
        }
      })
    }
  }
