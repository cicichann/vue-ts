...
export default {
  ...
  methods: {
    initProp () {
      this.data.columns = [{
        ...
        format: (row, prop) => {
          ...
          return  `<a class="title-link" href="#/ai/proof/?standardid=${row.standardId}&resourceid=${row.resourceId}&dataid=${row.dataId}" target="_blank">${content}</a>`
        }
      }, {
        label: '错别字',
        prop: 'wrongWords',
    ...
    queryList (page) {
      ...
      this.$api.queryCollectionCheck(params).then((data)=>{
