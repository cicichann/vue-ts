<template>
  <el-dialog :visible.sync="visible" :close-on-click-modal="false" :title="isModify ? '编辑分类属性字段' : '新增分类属性字段'">
...
export default {
  ...
  methods: {
    ...
    initData () {
      ...
      this.verifyCategoryHasData().then(() => {
    ...
    findFieldById () {
      ...
      this.verifyCategoryHasData().then(() => {
        ...
        this.$api.queryCategoryFieldData(url).then(data => {
     ...
     * 后端校验是否有数据
     */
    verifyCategoryHasData() {
      let url = {
        standardId: this.query.standardid
      }

      return new Promise((resolve, reject) => {
        this.$api.verifyCategoryHasData(url).then(data => {
    ...
    verifyInput (name, value, callback) {
      ...
      if (name === 'cnName') {
        this.$api.verifyCateCname(url, params).then((data) => {
          if(!data.available) {
        ...
        this.$api.verifyCateEname(url, params).then((data) => {
          if(!data.available) {
            callback(new Error(data.msg))
          }
          callback()
    ...
    addField (url, params) {
      this.loading = true
      this.$api.addCategoryField(url, params).then(data => {
        this.$trsModalSuccess('新增分类属性字段成功！')
        this.$close()
      }, () => {
        this.loading = false
      })
    ...
    editField(url, params) {
      this.loading = true
      this.$api.editCategoryField(url, params).then(data => {
        this.$trsModalSuccess('编辑分类属性字段成功！')
        this.$close()
      }, () => {
        this.loading = false
      })
