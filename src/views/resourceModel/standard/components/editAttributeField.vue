<template>
  <el-dialog :visible.sync="visible" :close-on-click-modal="false" :title="isModify ? '编辑字段属性字段' : '新增字段属性字段'">
    <el-form v-loading="loading" ref="form" :model="formData" :rules="rules" element-loading-text="数据加载中..." class="in-dialog" label-width="100px" label-suffix="：">
      ...
      <el-form-item class="in-input-row">
        ...
        <el-checkbox v-model="formData.isOverview">概览展示</el-checkbox>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    findFieldById () {
      ...
      this.verifyAttributeHasData().then(() => {
        let url = {
          standardId: this.query.standardid,
          columnId: this.columnId
        }

        this.$api.queryAttributeFieldData(url).then(data => {
          this.formData = Object.assign(data, {
             isRequired: data.isRequired === 1,
             isOverview: data.isOverview === 1,
             isUnique: false
          })
     ...
     * 后端校验是否有数据
     */
    verifyAttributeHasData() {
      let url = {
        standardId: this.query.standardid
      }

      return new Promise((resolve, reject) => {
        this.$api.verifyAttributeHasData(url).then(data => {
    ...
    verifyInput (name, value, callback) {
      ...
      if (name === 'cnName') {
        this.$api.verifyAttrCname(url, params).then((data) => {
          if (!data.available) {
        ...
        this.$api.verifyAttrEname(url, params).then((data) => {
          if(!data.available) {
            callback(new Error(data.msg))
          }
          callback()
    ...
    addField(url, params) {
      this.loading = true
      this.$api.addAttributeField(url, params).then(data => {
        this.$trsModalSuccess('新增字段属性字段成功！')
        this.$close()
      }, () => {
        this.loading = false
      })
    ...
    editField(url, params) {
      this.loading = true
      this.$api.editAttributeField(url, params).then(data => {
        this.$trsModalSuccess('编辑字段属性字段成功！')
        this.$close()
      }, () => {
        this.loading = false
      })
    ...
    confirm () {
      ...
      .then(() => {
        ...
        let params = {
          ...
          isOverview: this.formData.isOverview ? 1 : 0,
          isUnique: 0
        }
