  ...
  <div>
    ...
    <div class="sub-nav">
      <div class="fl">
        ...
        <button :disabled="data.selectedArray.length === 0" class="edit-btn other-btn" @click="downloadPic()">下载</button>
      </div>
      <div class="fr">
        <el-input v-model="data.searchValue" placeholder="请输入文件名" @keyup.enter.native="queryList">
          <i slot="suffix" class="input-icon" @click="searchPic"/>
        </el-input>
      </div>
    </div>
    ...
    <div class="sub-filter">
      <ul class="manage-tag fl">
        ...
        <router-link :to="{path: '/manage/inforepo/pic', query: $route.query}" title="图片" tag="li" class="tag-pic active"></router-link>
    ...
    <div class="table-content flex-table">
      ...
      <div v-loading="loading" :class="{'uirb-no-data': data.listSource.length === 0 && !loading}" class="card-wrap" element-loading-text="数据加载中..." style="margin: 10px 15px">
        <ul class="card-list clearfix">
          <li v-for="(item, $index) in data.listSource" :key="$index" class="info-repo-card">
            <el-checkbox v-model="data.selectedArray" :label="item.id" @click="selectedChange(item.id)"></el-checkbox>
            <el-popover :open-delay="500" placement="right" popper-class="info-repo-img-popover" trigger="hover">
              <template slot="default">
                <img :src="item.accessUrl" alt="" >
                <p class="desc overflow">{{ item.fileName }}</p>
              </template>
              <img slot="reference" :src="item.accessUrl" alt="" >
            </el-popover>
            <p class="desc overflow">
              <span>{{ item.fileName }}</span>
              <button style="display:none" class="list-icon download" title="下载" @click="downloadPic(item.id)"/>
            </p>
          </li>
        </ul>
      </div>
...
export default {
  name: 'ResourceManageInfoRepoPic',
  ...
  mounted() {
    if (sessionStorage.picFilter) this.status.selectedCategories = JSON.parse(sessionStorage.picFilter)
  ...
  methods: {
    ...
    queryList(page) {
      ...
      let url = (this.$route.query.appid ? `/apps/${this.$route.query.appid}` : `/standards/${this.$route.query.standardid}/categories`) +
          (this.$route.query.resourceid ? `/resources/${this.$route.query.resourceid}` : '') + '/pics'
    ...
    handleCategoryChange (category) {
      ...
      sessionStorage.picFilter = JSON.stringify(this.status.selectedCategories)
      this.queryList()
    },
     ...
     */
    downloadPic(id) {
      this.$trsModalConfirm({title: "下载图片", content: "是否确认下载所选图片？", danger: false}).then(() => {
        if (id) {
     ...
     */
    deletePic() {
      this.$trsModalConfirm({title: "删除图片", content: "是否确认删除所选图片？", danger: true}).then(() => {
        this.$api.deleteAttach({standardId: this.$route.query.standardid, appendixids: this.data.selectedArray.join(',')}, 'pics').then(() => {
          this.data.selectedArray.splice(0)
          this.$trsModalSuccess('删除图片成功!')
          this.queryList()
        })
      })
    },
