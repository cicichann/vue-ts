...
import editAttributeFieldDialog from './components/editAttributeField.vue'

export default {
  name: 'attributeFieldList',
  ...
  methods: {
    ...
    queryList() {
      this.loading = true
      let url = `/standards/${this.$route.query.standardid}/attribute/columns/transform`
    ...
    handleSortChange(params) {
      this.loading = true
      let url = `/standards/${this.$route.query.standardid}/attribute/columns/sort`
    ...
    editField (columnId = 0) {
      this.$dialog(editAttributeFieldDialog, {columnId: columnId, query: this.$route.query}).then(() => {
        this.queryList()
      })
    },
    ...
    deleteField(id) {
      this.$trsModalConfirm({
        ...
        let url = `/standards/${this.$route.query.standardid}/attribute/columns/${id}`
        this.$api.deleteField(url, params).then((data) => {
          this.$trsModalSuccess('删除字段属性字段成功!')
          this.queryList()
        })
      })
    }
