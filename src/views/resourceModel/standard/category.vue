    ...
    <div class="table-content no-selected" style="padding:0">
      <com-table
        ...
        <template slot-scope="item">
          <button v-if="!item.row.isSysDefault" class="list-icon edit-icon" title="编辑" @click="editField(item.row.id)"/>
          <button v-if="!item.row.isSysDefault" class="list-icon delete-icon" title="删除" @click="deleteField(item.row.id)"/>
        </template>
      </com-table>
    </div>
  </div>
...
import editCategoryFieldDialog from './components/editCategoryField.vue'
...
export default {
  ...
  methods: {
    ...
    queryList() {
      this.loading = true
      let url = `/standards/${this.$route.query.standardid}/category/columns/transform`
    ...
    handleSortChange(params) {
      this.loading = true
      let url = `/standards/${this.$route.query.standardid}/category/columns/sort`
    ...
    editField (columnId = 0) {
      this.$dialog(editCategoryFieldDialog, {columnId: columnId, query: this.$route.query}).then(() => {
        this.queryList()
      })
    },
    ...
    deleteField(id) {
      this.$trsModalConfirm({
        ...
        let url = `/standards/${this.$route.query.standardid}/category/columns/${id}`
        this.$api.deleteField(url, params).then((data) => {
          this.$trsModalSuccess('删除分类属性字段成功！')
          this.queryList()
        })
      })
    }
